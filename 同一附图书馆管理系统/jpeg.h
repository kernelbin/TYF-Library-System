...
static int info_JPEG(struct DLL_STRPICENV *env, int *info, int size, UCHAR *fp);
static int decode0_JPEG(struct DLL_STRPICENV *env, int size, UCHAR *fp, int b_type, UCHAR *buf, int skip);

static void jpeg_idct_init(int base_img[64][64]);
static int jpeg_init(JPEG *jpeg);
// int jpeg_header(JPEG *jpge);
static void jpeg_decode(JPEG * jpeg, UCHAR * rgb, int b_type);
...
// デコード
static void jpeg_decode_init(JPEG *jpeg)
{
	int i, j;
...
static void jpeg_idct(int *block, int *dest, int base_img[64][64])
{
	int i, j, k;
...
// ---- ブロックのデコード ---
// ハフマンデコード＋逆量子化＋逆ジグザグ
static int jpeg_decode_huff(JPEG *jpeg, int scan, int *block, UCHAR *zigzag_table)
{
...
// ブロック (補間かけるには、ここで)
// リサンプリング
static void jpeg_mcu_bitblt(int *src, int *dest, int width,
...
// MCU一個変換
static int jpeg_decode_mcu(JPEG *jpeg, UCHAR *zigzag_table)
{
	int scan, val;
	int unit, i, h, v;
	int *p, hh, vv;
...
static int jpeg_decode_yuv(JPEG *jpeg, int h, int v, unsigned char *rgb, int b_type)
{
	int x0, y0, x, y, x1, y1;
	int *py;
	int Y12, V;
...
static HBITMAP LoadJpegFromData(PBYTE pData,DWORD DataLength)
{
	HBITMAP hBitmap;
...
static HBITMAP LoadJpegFromFile(TCHAR File[])
{
	HANDLE hFile;
	DWORD FileLength, ReadLength;
	PBYTE pData;
	...
	pData = (PBYTE)malloc(FileLength);


	ReadFile(hFile, pData, FileLength, &ReadLength, NULL);
	...
	__try
	{
		if (info_JPEG(&env, info, FileLength, pData) == 0)
		{
			free(pData);
			CloseHandle(hFile);
			return 0;
	...
	if (decode0_JPEG(&env, FileLength, pData, 4, picbuf, 0) != 0)
	{
